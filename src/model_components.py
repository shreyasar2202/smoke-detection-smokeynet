...
class RawToTile_MobileNetFPNV3(nn.Module):
    """
    Description: MobileNetV3Large backbone with a Feature Pyramid Network a few linear layers.
    Args:
        - pretrain_backbone (bool): Pretrains backbone on ImageNet
    ...
    def forward(self, x, **kwargs):
        ...
        outputs = []
        for i in range(len(self.keys)):
            tile_outputs[self.keys[i]] = self.avgpool(tile_outputs[self.keys[i]])
            tile_outputs[self.keys[i]] = tile_outputs[self.keys[i]].view(batch_size * num_tiles * series_length, -1)
            outputs.append(tile_outputs[self.keys[i]])
        
        tile_outputs = torch.cat(outputs, dim=1) # [batch_size * num_tiles * series_length, 768]
        tile_outputs, embeddings = self.embeddings_to_output(tile_outputs, batch_size, num_tiles, series_length)
