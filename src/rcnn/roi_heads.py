...
#         if self.training: # TODO
        train_proposals, train_matched_idxs, train_labels, train_regression_targets = self.select_training_samples(proposals, targets)
#         else:
        labels = None
        regression_targets = None
        matched_idxs = None

        train_box_features = self.box_head(self.box_roi_pool(features, train_proposals, image_shapes))
        train_class_logits, train_box_regression = self.box_predictor(train_box_features)
        ...
        losses = {}
#         if self.training:
        assert train_labels is not None and train_regression_targets is not None
        loss_classifier, loss_box_reg = fastrcnn_loss(
            train_class_logits, train_box_regression, train_labels, train_regression_targets)
        losses = {
...
#             if self.training: # TODO
            assert train_matched_idxs is not None
            # during training, only focus on positive boxes
            num_images = len(train_proposals)
            train_mask_proposals = []
            train_pos_matched_idxs = []
            for img_id in range(num_images):
                pos = torch.where(train_labels[img_id] > 0)[0]
                train_mask_proposals.append(train_proposals[img_id][pos])
                train_pos_matched_idxs.append(train_matched_idxs[img_id][pos])
#             else:
            pos_matched_idxs = None

            if self.mask_roi_pool is not None:
                train_mask_features = self.mask_roi_pool(features, train_mask_proposals, image_shapes)
                train_mask_features = self.mask_head(train_mask_features)
                train_mask_logits = self.mask_predictor(train_mask_features)
...
#             if self.training:
            assert targets is not None
            assert train_pos_matched_idxs is not None
            assert train_mask_logits is not None

            gt_masks = [t["masks"] for t in targets]
            gt_labels = [t["labels"] for t in targets]
            rcnn_loss_mask = maskrcnn_loss(
                train_mask_logits, train_mask_proposals,
                gt_masks, gt_labels, train_pos_matched_idxs)
            loss_mask = {
